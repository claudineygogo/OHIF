...
class ScormPackageGenerator:
    ...
    def update_placeholders(self, package_dir, ohif_url):
        """Replaces {{CASE_URL}} placeholder in index.html and js/message-bridge.js."""
    ...
    def create_package(self, case_name, ohif_url):
        ...
        try:
            ...
            # 2. Update files
            self.update_placeholders(temp_dir, ohif_url)
...
def get_gui_inputs():
    """
    Displays a GUI window to collect Case Name and OHIF URL.
    Returns a tuple (case_name, ohif_url) or (None, None) if cancelled.
    """
    root = tk.Tk()
    ...
    inputs = {"case_name": None, "ohif_url": None}
    ...
    def on_submit():
        ...
        if not case or not url:
            messagebox.showerror("Error", "Both Case Name and URL are required.")
            return
    ...
    return inputs["case_name"], inputs["ohif_url"]

def main():
    try:
        # 1. Get Inputs via GUI
        case_name, ohif_url = get_gui_inputs()
        
        if not case_name or not ohif_url:
        ...
        zip_path = generator.create_package(case_name, ohif_url)
