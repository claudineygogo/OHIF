...
function commandsModule({
  ...
  const actions = {
    ...
    addContourStampSegment: async ({
      ...
      const enabledElement =
        cornerstoneViewportService.getCornerstoneViewport(activeViewportId)?.element;

      if (!enabledElement) {
        return;
      }

      const segmentLabelToUse = segmentLabel;
      ...
      const segmentation = segmentationService.getSegmentation(segmentationId);
      const segmentIndex = segmentationService.getNextAvailableSegmentIndex(segmentationId);

      segmentationService.addSegment(segmentationId, {
        segmentIndex,
        properties: {
          label: segmentLabelToUse,
          color: [255, 255, 0, 255],
          isLocked: false,
          isVisible: true,
        },
      ...
      const annotationData = {
        ...
        metadata: {
          ...
          FrameOfReferenceUID: metaData.get('FrameOfReferenceUID', imageId),
        },
        highlighted: false,
        autoGenerated: false,
        isLocked: false,
      ...
      cornerstoneTools.annotation.state.addAnnotation(annotationData, enabledElement);

      cstUtils.triggerAnnotationRenderForViewportIds([activeViewportId]);
    },
    ...
    setDynamicCursorSizeForSculptorTool: ({ value: isDynamicCursorSize }) => {
      const viewportId = viewportGridService.getActiveViewportId();
      const toolGroup = toolGroupService.getToolGroupForViewport(viewportId);
      const toolInstance = toolGroup.getToolInstance(toolNames.SculptorTool);
    ...
    setStampRadius: ({ value: radiusMM, toolNames }) => {
      const viewportId = viewportGridService.getActiveViewportId();
      const toolGroup = toolGroupService.getToolGroupForViewport(viewportId);
  ...
  const definitions = {
    ...
    setStampRadius: actions.setStampRadius,
    ...
    addContourStampSegment: {
      commandFn: actions.addContourStampSegment,
    },
  };
